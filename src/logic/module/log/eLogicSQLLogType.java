...
public enum eLogicSQLLogType implements LogType {
    ...
    public String Serialize() {
        switch (this) {
            case LOGIC_SQL_CREATE_ROLE:
                return SQLBuild.GetInstance().GetSQL(new CreateRoleLog());
            case LOGIC_SQL_LOGIN:
                return SQLBuild.GetInstance().GetSQL(new LoginLog());
            case LOGIC_SQL_LOGOUT:
                return SQLBuild.GetInstance().GetSQL(new LogoutLog());
            case LOGIC_SQL_REGISTER:
                return SQLBuild.GetInstance().GetSQL(new RegisterLog());
            case LOGIC_SQL_OPERATION:
                return SQLBuild.GetInstance().GetSQL(new OperationLog());
            case LOGIC_SQL_ROOMINFO:
                return SQLBuild.GetInstance().GetSQL(new roominfolog());
            case LOGIC_SQL_ROOMCARD:
                return SQLBuild.GetInstance().GetSQL(new roomcardlog());
