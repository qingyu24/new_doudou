...
public static class _ConnectionManager
{
	...
	static public void RoomImpl_removePlayer(this ConnectionManager mgr, int targetID, int teamID)
	{
		PacketBuffer.GetInstance().Clear().SetObjMethod(1,20).Add(targetID).Add(teamID).Send(mgr);
	}
	...
	static public void RoomImpl_visitGame(this ConnectionManager mgr, long roleID)
	{
		PacketBuffer.GetInstance().Clear().SetObjMethod(1,37).Add(roleID).Send(mgr);
	}
	...
	{
		PacketBuffer.GetInstance().Clear().SetObjMethod(3,3).Add(p_ID).Add(p_teamID).Add(m_friendID).Add(roomID).Add(ifFree).Send(mgr);
	}

	static public void CenterImpl_creatRoomRule(this ConnectionManager mgr, int isTeam, int gameTime, int teNumber, int eachSize, string roomName, int roomPass)
	{
		PacketBuffer.GetInstance().Clear().SetObjMethod(3,13).Add(isTeam).Add(gameTime).Add(teNumber).Add(eachSize).Add(roomName).Add(roomPass).Send(mgr);
	}
	...
	static public void CenterImpl_visitPlayer(this ConnectionManager mgr, int roomID)
	{
		PacketBuffer.GetInstance().Clear().SetObjMethod(3,8).Add(roomID).Send(mgr);
	}
	...
	static public void CenterDateImpl_changeSkin(this ConnectionManager mgr, int list, int buffer)
	{
		PacketBuffer.GetInstance().Clear().SetObjMethod(7,10).Add(list).Add(buffer).Send(mgr);
	}
	...
	static public void CenterDateImpl_rankingClass(this ConnectionManager mgr, int this, int p_user, int list_type)
	{
		PacketBuffer.GetInstance().Clear().SetObjMethod(7,9).Add(this).Add(p_user).Add(list_type).Send(mgr);
	}

	static public void CenterDateImpl_searchUser(this ConnectionManager mgr, string this)
	{
		PacketBuffer.GetInstance().Clear().SetObjMethod(7,11).Add(this).Send(mgr);
	}
