...
public static class _ConnectionManager
{
	...
	static public void CenterImpl_creatRoomRule(this ConnectionManager mgr, int p_user, int isTeam, int gameTime, int teNumber, string eachSize, int roomName)
	{
		PacketBuffer.GetInstance().Clear().SetObjMethod(3,13).Add(p_user).Add(isTeam).Add(gameTime).Add(teNumber).Add(eachSize).Add(roomName).Send(mgr);
	}
